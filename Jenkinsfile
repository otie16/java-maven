        ...
        stage('build image'){
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'dockerhub-repo-credentials', passwordVariable: 'PASS', usernameVariable: 'USERNAME', passwordVariable: 'USER')])
                        sh 'docker build -t otobongedoho18361/demo-app:jma-2.0 .'
                        sh 'echo $PASS | docker login -u $USER --password-stdin'
                        sh 'docker push otobongedoho18361/demo-app:2.0'
                }
            }
        }
        stage('deploy'){
